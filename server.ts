...
app.post("/api/messages/:userId", async (req, res) => {
  ...
  if (containsRedFlag) {
    const alertMessage: Message = {
      ...
      patient_id: userId,
      sender: "ai",
      text: "!!!EMERGENCY ALERT!!! Please stop exercising immediately and call 911 or your local emergency services. You are experiencing critical warning signs that require urgent medical attention. Do not try to complete any more exercises or wait for symptoms to pass.",
      timestamp: Date.now(),
      is_alert: true,
    };
  ...
  if (!ai) {
    ...
    aiText = `Hi there! I am your rehabilitation assistant. Thank you for sharing. Based on your mobility level (${profile.mobility_level}) and stage (${profile.recovery_stage}), please make sure you always listen to your body and sit down if you feel tired. We have logged ${completedToday.length || "no"} exercises today. Let me know if you need specific exercise instructions!`;
  } else {
    try {
